...
2. Execute make-preseed.iso.sh. The netinst debian image is repacked with the final preseed file
...
1. Execute "./render.sh compute" to render the answer.toml with the secrets necessary
2. Download latest proxmox ISO 
3. Install proxmox-auto-install-assistant or use the dockerized utility
...
We need to setup API authentication in proxmox to allow for Terraform to interact with the API. Through SSH, ansible playbook will execute the following commands:
