...
locals {
  k8s_nodes = {
    "k8s-cp-1" = { octet = 193, cores = 2, memory = 6144, role = "control" }
    "k8s-w-1"  = { octet = 194, cores = 2, memory = 8192, role = "worker" }
    "k8s-w-2"  = { octet = 195, cores = 2, memory = 8192, role = "worker" }
    "k8s-w-3"  = { octet = 197, cores = 2, memory = 8192, role = "worker" }
  }
}
...
resource "proxmox_download_file" "debian_13_genericcloud_amd64" {
  ...
  datastore_id = "local"
  node_name    = "pve"
  url          = "https://laotzu.ftp.acc.umu.se/cdimage/cloud/trixie/latest/debian-13-genericcloud-amd64.qcow2"
}